...
const CartReducer = (state = initState, action) => {
    switch (action.type) {
        ...
        case actions.LOAD_ADD_CART_SYNC_PRODUCT_SUCCESS:
            let dataArray = [...state.data.cartProducts];

            let indexAdd = dataArray.findIndex(v => v.ProductId == action.payload.ProductId
                && v.ColorId == action.payload.ColorId
                && v.SizeId == action.payload.SizeId);

            if (indexAdd == -1) {
                dataArray.push(action.payload);
            } else {
                dataArray[indexAdd].Quantity += action.payload.Quantity;
            }
            return {
                ...state,
                data: {
                    ...state.data,
                    cartProducts: dataArray
                }
            }
        case actions.LOAD_UPDATE_CART_SYNC_PRODUCT_SUCCESS:
            let dataArrayUpdate = [...state.data.cartProducts];

            let indexUpdate = dataArrayUpdate.findIndex(v => v.ProductId == action.payload.ProductId
                && v.ColorId == action.payload.ColorId
                && v.SizeId == action.payload.SizeId);

            if (indexUpdate != -1) {
                dataArrayUpdate[indexAdd] = action.payload;
            }
            return {
                ...state,
                data: {
                    ...state.data,
                    cartProducts: dataArrayUpdate
                }
            }
        case actions.LOAD_DELETE_CART_SYNC_PRODUCT_SUCCESS:
            let dataArrayDelete = [...state.data.cartProducts];
            let dataCartChecked = [...state.data.checkedCart];

            let indexDelete = dataArrayDelete.findIndex(v => v.ProductId == action.payload.ProductId
                && v.ColorId == action.payload.ColorId
                && v.SizeId == action.payload.SizeId);

            if (indexDelete != -1) {
                dataArrayDelete.splice(indexDelete,1);
            }
            let indexDeleteCartChecked=dataCartChecked.findIndex(v=>v.ColorId == action.payload.ColorId 
                && v.ProductId == action.payload.ProductId 
                && v.SizeId == action.payload.SizeId );

            if(indexDeleteCartChecked != -1){
                dataCartChecked.splice(indexDeleteCartChecked,1)
            }
            return {
                ...state,
                data: {
                    ...state.data,
                    cartProducts: dataArrayDelete,
                    checkedCart : dataCartChecked
                }
            }
        default:
            return {
